...
W <-c("ARIZONA","CALIFORNIA", "COLORADO",
       "IDAHO", "MONTANA", "NEVADA", "OREGON",
       "NEWMEXICO", "UTAH","WASHINGTON", "WYOMING") 

# get confirmed cases, N and spatial data for this region

confirmed_W <- confirmed_cases[,W]

N_W <- census_data[grepl(paste(W, collapse="|"), census_data$X1),] %>% 
  subset(., select = c("X13"))

spatial_W <- spatial_data[rownames(spatial_data)%in%W,colnames(spatial_data)%in%W]
...
data_model_W = DataModel(Y=apply(confirmed_W, 2,cumsum), 
...
W.locations <- ncol(spatial_W)
W.timepoints <- length(unique(states_new_cases$weeks))

intercepts_W = diag(11)[rep(1:ncol(confirmed_W), each = nrow(confirmed_W)),]
#intercepts = 1
timeBasis_W = bs(1:nrow(confirmed_W), degree = 3)[rep(1:nrow(confirmed_W), ncol(confirmed_W)),]

X = cbind(intercepts_W, timeBasis_W)

exposure_model_W = ExposureModel(X, nTpt = nrow(confirmed_W),
                               nLoc = ncol(confirmed_W),
                               betaPriorPrecision = 0.5,
                               betaPriorMean = c(rep(-1, ncol(intercepts_W)),
                                                 rep(0, ncol(timeBasis_W))))
...
epidemic.start_W = min(which(apply(confirmed_W, 1, max) > 0)) # starts in week 1 or epoch week 3
confirmed_W = confirmed_W[(epidemic.start_W-1):nrow(confirmed_W),]

I0_W = (apply(confirmed_W[1:3,], 2, max) > 0)*2
E0_W = I0_W
R0_W = 0*I0_W
S0_W = as.numeric(unlist(N_W-E0_W-I0_W-R0_W))

initial_values_W = InitialValueContainer(S0 = S0_W, 
                                             E0 = E0_W,
                                             I0 = I0_W,
                                             R0 = R0_W)
```
...
CAR_model_W <- DistanceModel(list(spatial_W), 
                                 priorAlpha = 1,
                                 priorBeta = 1
                                 )
...
runtime_W = system.time(result_W <- SpatialSEIRModel(data_model_W,
                                                    exposure_model_W,
                                                    reinfection_model,
                                                    CAR_model_W,
...
```{r}
makePlots_W <- function(modelObj, nm){
  ...
  plotLocation <- function(x, model){
    plot(confirmed_W[,x], ylim = c(0, max(Isub[,x])),
         main = paste(model, ": Observed and Posterior Predictive Simulation.\n location ", 
                      colnames(spatial_W)[x], sep = ""))
    ...
    #})
    points(confirmed_W[,x], pch = 16, col = "blue")
  }
  
  for (i in 1:ncol(spatial_W)){
    plotLocation(i, nm)
  }
}
```
...
```{r}
runtime_W_plots = system.time(W_plots <- makePlots_W(result_W, "Dist"))
