...
S <-c("ALABAMA","ARKANSAS", "DELAWARE", "FLORIDA",
       "KENTUCKY", "GEORGIA", "MARYLAND", "MISSISSIPPI",
       "NORTHCAROLINA", "SOUTHCAROLINA","TENNESSEE", "VIRGINIA", "WASHINGTON", "WESTVIRGINIA") 

# get confirmed cases, N and spatial data for this region

confirmed_S <- confirmed_cases[,S]

N_S <- census_data[grepl(paste(S, collapse="|"), census_data$X1),] %>% 
  subset(., select = c("X13"))

spatial_S <- spatial_data[rownames(spatial_data)%in%S,colnames(spatial_data)%in%S]
...
data_model_S = DataModel(Y=apply(confirmed_S, 2,cumsum), 
...
S.locations <- ncol(spatial_S)
S.timepoints <- length(unique(states_new_cases$weeks))

intercepts_S = diag(14)[rep(1:ncol(confirmed_S), each = nrow(confirmed_S)),]
#intercepts = 1
timeBasis_S = bs(1:nrow(confirmed_S), degree = 3)[rep(1:nrow(confirmed_S), ncol(confirmed_S)),]

X = cbind(intercepts_S, timeBasis_S)

exposure_model_S = ExposureModel(X, nTpt = nrow(confirmed_S),
                               nLoc = ncol(confirmed_S),
                               betaPriorPrecision = 0.5,
                               betaPriorMean = c(rep(-1, ncol(intercepts_S)),
                                                 rep(0, ncol(timeBasis_S))))
...
epidemic.start_S = min(which(apply(confirmed_S, 1, max) > 0)) # starts in week 1 or epoch week 3
confirmed_S = confirmed_S[(epidemic.start_S-1):nrow(confirmed_S),]

I0_S = (apply(confirmed_S[1:3,], 2, max) > 0)*2
E0_S = I0_S
R0_S = 0*I0_S
S0_S = as.numeric(unlist(N_S-E0_S-I0_S-R0_S))

initial_values_S = InitialValueContainer(S0 = S0_S, 
                                             E0 = E0_S,
                                             I0 = I0_S,
                                             R0 = R0_S)
```
...
#spatial_S <- ( spatial_S == 1)*1
CAR_model_S <- DistanceModel(list(spatial_S), 
                                 priorAlpha = 1,
                                 priorBeta = 1
                                 )
...
runtime_S = system.time(result_S <- SpatialSEIRModel(data_model_S,
                                                    exposure_model_S,
                                                    reinfection_model,
                                                    CAR_model_S,
...
```{r}
makePlots_S <- function(modelObj, nm){
  ...
  plotLocation <- function(x, model){
    plot(confirmed_S[,x], ylim = c(0, max(Isub[,x])),
         main = paste(model, ": Observed and Posterior Predictive Simulation.\n location ", 
                      colnames(spatial_S)[x], sep = ""))
    ...
    #})
    points(confirmed_S[,x], pch = 16, col = "blue")
  }
  
  for (i in 1:ncol(spatial_S)){
    plotLocation(i, nm)
  }
}
```
...
```{r}
runtime_S_plots = system.time(S_plots <- makePlots_S(result_S, "Dist"))
