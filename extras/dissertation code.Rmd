...
data_model = DataModel(Y=apply(confirmed_NE, 2,cumsum), 
...
n.locations <- ncol(spatial_NE)
n.timepoints <- length(unique(states_new_cases$weeks))

intercepts = diag(20)[rep(1:ncol(confirmed_NE), each = nrow(confirmed_NE)),]
#intercepts = 1
timeBasis = bs(1:nrow(confirmed_NE), degree = 3)[rep(1:nrow(confirmed_NE), ncol(confirmed_NE)),]

X = cbind(intercepts, timeBasis)

exposure_model = ExposureModel(intercepts, nTpt = nrow(confirmed_NE),
...
epidemic.start_NE = min(which(apply(confirmed_NE, 1, max) > 0)) # starts in week 1 or epoch week 3
...
initial_values = InitialValueContainer(S0 = S0_NE, 
                                             E0 = E0_NE,
                                             I0 = I0_NE,
                                             R0 = R0_NE)
```
...
CAR_model <- DistanceModel(list(spatial_NE), 
                                 priorAlpha = 1,
                                 priorBeta = 1
                                 )
...
simulations_fl <- epidemic.simulations(result_fl, replicates = 10)
