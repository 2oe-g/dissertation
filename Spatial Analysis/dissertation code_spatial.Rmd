...
vaccines_dates <- vaccines_timeline_sub %>%
  ...
  filter(date >= "2021-06-01") # filter by this date as this is when boosters first administered and data becomes more sparse around this time (also marks two years since first case)
```
...
# scale alabama by larger value as it has less cases compared to florida and georgia...prevents epidemic from dying out by fixing intensity to acceptable scale (since intensity is estimated linearly)
...
```{r}
plotPosteriorPredictive_compare_dm = function(m1a, m1b, m1c, nm)  
{
  Is_m1a <- lapply(m1a$simulationResults, function(x){x$I_star})
  Is_m1a <- array(Reduce(c, Is_m1a), dim = c(nrow(Is_m1a[[1]]),
                                           ncol(Is_m1a[[2]]),
                                           length(Is_m1a)))
  
  Ism_m1a <- apply(Is_m1a, 1:2, mean)
  Islb_m1a <- apply(Is_m1a, 1:2, quantile, probs = c(0.025))
  Isub_m1a <- apply(Is_m1a, 1:2, quantile, probs = c(0.975))
  
  Is_m1b <- lapply(m1b$simulationResults, function(x){x$I_star})
  Is_m1b <- array(Reduce(c, Is_m1b), dim = c(nrow(Is_m1b[[1]]),
                                           ncol(Is_m1b[[2]]),
                                           length(Is_m1b)))
  
  Ism_m1b <- apply(Is_m1b, 1:2, mean)
  Islb_m1b <- apply(Is_m1b, 1:2, quantile, probs = c(0.025))
  Isub_m1b <- apply(Is_m1b, 1:2, quantile, probs = c(0.975))
  
  Is_m1c <- lapply(m1c$simulationResults, function(x){x$I_star})
  Is_m1c <- array(Reduce(c, Is_m1c), dim = c(nrow(Is_m1c[[1]]),
                                           ncol(Is_m1c[[2]]),
                                           length(Is_m1c)))
  
  Ism_m1c <- apply(Is_m1c, 1:2, mean)
  Islb_m1c <- apply(Is_m1c, 1:2, quantile, probs = c(0.025))
  Isub_m1c <- apply(Is_m1c, 1:2, quantile, probs = c(0.975))
  
  plotLocation <- function(x, model){
    plot(first_wave_confirmed_sub[,x], ylim = c(0, max(Ism_m1a[,x], Ism_m1b[,x], Ism_m1c[,x])*1.75), xlab = "Epidemic Day", ylab = "New Cases",
         main = paste(model, "\n location ", 
                      colnames(spatial_sub)[x], sep = ""))
    lines(Ism_m1a[,x], lty = 1, col = "blue")
    lines(Islb_m1a[,x], lty = 2, col = "blue")
    lines(Isub_m1a[,x], lty = 2, col = "blue")
    
    lines(Ism_m1b[,x], lty = 1, col = "red")
    lines(Islb_m1b[,x], lty = 2, col = "red")
    lines(Isub_m1b[,x], lty = 2, col = "red")
    
    lines(Ism_m1c[,x], lty = 1, col = "chartreuse2")
    lines(Islb_m1c[,x], lty = 2, col = "chartreuse2")
    lines(Isub_m1c[,x], lty = 2, col = "chartreuse2")
    
    legend("topleft", legend = c("Mean", "95% CI", "Observed", "Model 1a", "Model 1b",
                               "Model 1c"), 
         lty = c(1,2,0,1,1,1), 
         pch = c(NA,NA,1,NA,NA,NA), col = c("black", "black", "black", "blue", "red", "chartreuse2"), cex = 1)
  }
...
plotPosteriorPredictive_compare_t1_alg = function(simulations_exp_SMC, simulations_wei_SMC, 
                                                  simulations_exp_basic, simulations_wei_basic, nm)  
{
  
  Is_exp_SMC <- lapply(simulations_exp_SMC$simulationResults, function(x){x$I_star})
  Is_exp_SMC <- array(Reduce(c, Is_exp_SMC), dim = c(nrow(Is_exp_SMC[[1]]),
                                           ncol(Is_exp_SMC[[2]]),
                                           length(Is_exp_SMC)))
  
  Ism_exp_SMC <- apply(Is_exp_SMC, 1:2, mean)
  Islb_exp_SMC <- apply(Is_exp_SMC, 1:2, quantile, probs = c(0.025))
  Isub_exp_SMC <- apply(Is_exp_SMC, 1:2, quantile, probs = c(0.975))
  
  
  Is_wei_SMC <- lapply(simulations_wei_SMC$simulationResults, function(x){x$I_star})
  Is_wei_SMC <- array(Reduce(c, Is_wei_SMC), dim = c(nrow(Is_wei_SMC[[1]]),
                                           ncol(Is_wei_SMC[[2]]),
                                           length(Is_wei_SMC)))
  
  Ism_wei_SMC <- apply(Is_wei_SMC, 1:2, mean)
  Islb_wei_SMC <- apply(Is_wei_SMC, 1:2, quantile, probs = c(0.025))
  Isub_wei_SMC <- apply(Is_wei_SMC, 1:2, quantile, probs = c(0.975))
  
  
  Is_exp_basic <- lapply(simulations_exp_basic$simulationResults, function(x){x$I_star})
  Is_exp_basic <- array(Reduce(c, Is_exp_basic), dim = c(nrow(Is_exp_basic[[1]]),
                                           ncol(Is_exp_basic[[2]]),
                                           length(Is_exp_basic)))
  
  Ism_exp_basic <- apply(Is_exp_basic, 1:2, mean)
  Islb_exp_basic <- apply(Is_exp_basic, 1:2, quantile, probs = c(0.025))
  Isub_exp_basic <- apply(Is_exp_basic, 1:2, quantile, probs = c(0.975))
  
  
  Is_wei_basic <- lapply(simulations_wei_basic$simulationResults, function(x){x$I_star})
  Is_wei_basic <- array(Reduce(c, Is_wei_basic), dim = c(nrow(Is_wei_basic[[1]]),
                                           ncol(Is_wei_basic[[2]]),
                                           length(Is_wei_basic)))
  
  Ism_wei_basic <- apply(Is_wei_basic, 1:2, mean)
  Islb_wei_basic <- apply(Is_wei_basic, 1:2, quantile, probs = c(0.025))
  Isub_wei_basic <- apply(Is_wei_basic, 1:2, quantile, probs = c(0.975))
  
   
  
  plotLocation <- function(x, model){
    plot(first_wave_confirmed_sub[,x], ylim = c(0, max(first_wave_confirmed_sub[,x])*1.5),
         xlab = "Epidemic Day", ylab = "New Cases",
         main = paste(model, "\n location ", 
                      colnames(spatial_sub)[x], sep = ""))
    lines(Ism_exp_SMC[,x], lty = 1, col = "blue")
    lines(Islb_exp_SMC[,x], lty = 2, col = "blue")
    lines(Isub_exp_SMC[,x], lty = 2, col = "blue")
    
    lines(Ism_wei_SMC[,x], lty = 1, col = "red")
    lines(Islb_wei_SMC[,x], lty = 2, col = "red")
    lines(Isub_wei_SMC[,x], lty = 2, col = "red")
    
    lines(Ism_exp_basic[,x], lty = 1, col = "chartreuse2")
    lines(Islb_exp_basic[,x], lty = 2, col = "chartreuse2")
    lines(Isub_exp_basic[,x], lty = 2, col = "chartreuse2")
    
    lines(Ism_wei_basic[,x], lty = 1, col = "darkgoldenrod1")
    lines(Islb_wei_basic[,x], lty = 2, col = "darkgoldenrod1")
    lines(Isub_wei_basic[,x], lty = 2, col = "darkgoldenrod1")
...
```{r}
plotPosteriorPredictive_compare_t2_alg = function(simulations_exp_SMC, simulations_wei_SMC, 
                                                  simulations_exp_basic, simulations_wei_basic, nm)  
{
  ...
  plotLocation <- function(x, model){
    plot(firstandsecond_wave_confirmed_sub[,x], ylim = c(0, max(firstandsecond_wave_confirmed_sub[,x])*1.5),
         xlab = "Epidemic Day", ylab = "New Cases",
         main = paste(model, "\n location ", 
                      colnames(spatial_sub)[x], sep = ""))
    lines(Ism_exp_SMC[,x], lty = 1, col = "blue")
...
runtime1a_basic= system.time(result1a_basic <- SpatialSEIRModel(data_model_1,
                                                   ...
                                                   #weibull_transition_priors,
                                                    ...
                                                    samples = 100,
                                                    verbose = 2)) 
runtime1a_basic_wei= system.time(result1a_basic_wei <- SpatialSEIRModel(data_model_1,
                                                    exposure_model_1,
                                                    reinfection_model,
                                                    distance_model,
                                                   weibull_transition_priors,
...
simulations1a_basic_wei <- epidemic.simulations(result1a_basic_wei, replicates = 50)
...
```{r}
plotPosteriorPredictive_compare = function(simulations_exp_SMC, simulations_wei_SMC, rawData, nm, lastTime)  
{
  ...
  plotLocation <- function(x, model){
    plot(rawData[,x], ylim = c(0, max(rawData[,x])*2.5),
         xlab = "Epidemic Day", ylab = "New Cases",
         main = paste(model, "\n location ", 
                      colnames(spatial_sub)[x], sep = ""),
         col = ifelse(1:length(rawData[,x]) <= lastTime, "black", "red"))
    ...
    lines(Ism_wei_SMC[,x], lty = 1, col = "chartreuse2")
    lines(Islb_wei_SMC[,x], lty = 2, col = "chartreuse2")
    lines(Isub_wei_SMC[,x], lty = 2, col = "chartreuse2")

    
    legend("topleft", legend = c("Mean", "95% CI", "Observed", "Future","Exponential (SMC)", "Weibull (SMC)"), 
         lty = c(1,2,0,0,1,1), 
         pch = c(NA,NA,1,1,NA,NA), col = c("black", "black", "black", "red", "blue","chartreuse2"), cex = 1)
  }
...
```{r}
timeMatrix = rbind(runtime1a, runtime1a_weibull, runtime1a_basic, runtime1a_basic_wei, 
                   runtime1b, runtime1c, 
                   runtime2a, runtime2a_weibull, runtime2a_basic, runtime2a_weibull_basic, 
                   runtime2b, runtime2c, 
