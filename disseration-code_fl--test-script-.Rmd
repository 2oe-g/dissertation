...
3. Lack of intensity parameters results in diminshed estimates...use of splines improves fit (demonstrate differing dfs)
...
vaccine_prop <- data.frame(prop_vaxxed = latest_vaxxed/21477737,
                                        ...
                                        #prop_boosted = latest_boosted/21477737)


exposure.design.matrix <- as.matrix(
                            cbind(1,
                                  #sapply(vaccine_prop, rep.int, times = nrow(vaccines_dates))
                                  X_vaccines,
                                  #time_basis_vaccines,
                              vaccine_prop
...
exp_transition_priors = ExponentialTransitionPriors(p_ei = 1-exp(-1/6), 
                                                    # cdf of exponential dist...dont have actual value of gamma so estimate 
                                                    # using mean (gamma) = 1/days......exp(rate parameter = 1/6)
                                                      #1-exp(-1/5.68),
...
```{r}
#runtime10 = system.time({
  #result10 <- SpatialSEIRModel(data_model = data_model_interventions,
  #                                 exposure_model = exposure_model_6,
  #                                 reinfection_model = reinfection_model,
  #                                 distance_model = distance_model,
  #                                 transition_priors = exp_transition_priors,
  #                                 initial_value_container = initial_values,
  #                                 sampling_control = sampling_control.init,
  #                                 samples = 250, 
  #                                 verbose = FALSE)
  #result10 <- update(result10, 
  #                       sampling_control = sampling_control_update, 
  #                       verbose=FALSE)
...
{
  allSimulatedI_star = sapply(simulations$simulationResults, function(x){x$p_ei})
...
{
  allSimulatedI_star = sapply(simulations$simulationResults, function(x){x$p_ir})
...
# two interventions plus time basis
```
```{r}
...
# two interventions, reopen and time basis
```
```{r}
...
# two interventions, reopen and trig function
```
```{r}
