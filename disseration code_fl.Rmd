...
vaccines_dates <- fl %>%
  filter(date >= "2021-10-21" & date <=latest_date)
...
data_model_full = DataModel(Y=fl$cases, 
                             type = "identity",      # Assume data is correct 
                             compartment = "I_star", # Data related to new infections
                             cumulative = FALSE       # Not reported on cumulative scale
                             )
...
time_basis = bs(1:nrow(first_wave), degree = 4)[rep(1:nrow(first_wave), 1),]
...
reopen_date = as.Date("2021-05-03", "%Y-%m-%d") # signed mandate for statewide removal of all covid interventions
  #as.Date("2020-07-01", "%Y-%m-%d")

time_basis_interventions = bs(1:nrow(firstandsecond_wave), degree = 4)[rep(1:nrow(firstandsecond_wave), 1),]
...
```
```{r}
time_basis_full = bs(1:nrow(fl), degree = 4)[rep(1:nrow(fl), 1),]

X_shift_full <- cbind(1, 
       1*(fl$date >= intervention_date), 
       1*(fl$date >= reopen_date))
intervention_matrix_shift = cbind(X_shift_full, time_basis_full)

c1_full <- cumsum(1*((fl$date >= intervention_date)))
sharedBasis_full <- bs(0:200, degree = 4) 
X_splines_full <- as.matrix(cbind(1,predict(sharedBasis_full,c1_full)))

exposure_model_full = ExposureModel(time_basis_full,
  #cbind(1,intervention_term),
                                nTpt = nrow(fl),
                                nLoc = 1,
                                betaPriorPrecision = 0.5,
                                betaPriorMean = c(rep(-3.1, ncol(time_basis_full))))
```
...
time_invariant_covariates <- data.frame(prop_vaxxed = vaccinations_fl$people_vaccinated/21477737,
                                        prop_fully_vaxxed = vaccinations_fl$people_fully_vaccinated/21477737,
                                        prop_boosted = vaccinations_fl$total_boosters/21477737)
...
exposure_model_5 = ExposureModel(X = exposure.design.matrix,
                                      ...
                                      betaPriorMean = c(rep(0.4118, ncol(time_invariant_covariates)), # researched prior mean
